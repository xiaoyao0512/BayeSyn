...
class Synthesis_old:
	...
	def rewrite(self, program):
		...
		if (prob <= probRepOpr):
		...
		elif (prob > probRepOpr and prob <= (probRepOpr+probRepOprs)):
			print 'Case 2: replace all operands'
			# pick a new opcode
			newQubits = []
			...
			#gateList = gates.Gates.univGates(program.type2)
			#randOpc = random.randint(0, len(gateList)-1)
			#program.qxCode[randItr].type = gateList[randOpc]
			#print gateList[randOpc]
		elif (prob > (probRepOpr+probRepOprs) and prob <= (probRepOpr+probRepOprs+probRepIst)):
		...
		elif (prob > (probRepOpr+probRepOprs+probRepIst) and prob <= (probRepOpr+probRepOprs+probRepIst+probRmv)):
			print 'Case 4: remove a random instruction'
			program.qxCode.remove(randOp)

		elif (prob > (probRepOpr+probRepOprs+probRepIst+probRmv) and prob <= (probRepOpr+probRepOprs+probRepIst+probRmv+probRndRst)):
			print 'Case 5: random restart'
			program.qxCode = []
			program.registers = []
			program.genGates()

		elif (prob > (probRepOpr+probRepOprs+probRepIst+probRmv+probRndRst) and prob <= (probSwap+probRepOpr+probRepOprs+probRepIst+probRmv+probRndRst)):
			print 'Case 6: Swap two instructions'
	...
	def synthesize(self):
		...
		costArray = []
		alphaArray = []
		while (iterations < self.iterations and currErr > self.err):
